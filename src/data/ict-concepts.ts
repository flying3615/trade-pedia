...
export const ictConcepts: ICTConcept[] = [
    ...
    },
    {
        ...
        explanation: {
            overview: '两种场景都出现了MSS（市场结构转换），但结果截然不同。关键在于FVG填补后趋势是否真正逆转。\n' +
                '\n' +
                '核心问题：\n' +
                '• 为什么同样有MSS，一个成功逆转，一个继续原趋势？\n' +
                '• 答案在于HTF（更高时间框架）的状态\n' +
                '\n' +
                '两种场景对比：\n' +
                '• 成功案例：MSS + FVG回补 → 趋势成功逆转（从多转空）\n' +
                '• 失败案例：MSS + FVG回补 → 但HTF扩张 → 价格继续原趋势\n' +
                '\n' +
                '核心教训：\n' +
                '• MSS本身不足以确认趋势逆转\n' +
                '• 必须检查HTF是否支持这个逆转\n' +
